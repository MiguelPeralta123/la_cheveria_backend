...
export const createProd = async (req, res) => {
  const { nombre, descripcion, marca, urlImagen, precio, cantidad, noSerieProd } =
    req.body;
  ...
  try {
    const [rows] = await pool.query('INSERT INTO productos (nombre, descripcion, marca, urlImagen, precio, cantidad, noSerieProd) VALUES (?, ?, ?, ?, ?, ?, ?)', [nombre, descripcion, marca, urlImagen, precio, cantidad, noSerieProd]);
    res.send({ nombre, descripcion, marca, urlImagen, precio, cantidad, noSerieProd });
  } catch (error) {
    res.status(500);
    res.send(error.message);
  }
